...
const UI_KEY = "math-league-clean-v3-ui";
...
function loadUiState(): UiState {
  ...
  try {
    return { ...DEFAULT_UI, ...JSON.parse(raw) } as UiState;
  } catch {
    return DEFAULT_UI;
  }
}
...
export default function App() {
  ...
  const [screen, setScreen] = useState<Screen>(ui.screen);
  const [mode, setMode] = useState<Mode>(ui.mode);
  const [questionSeed, setQuestionSeed] = useState(Date.now() % 100000);
  const [question, setQuestion] = useState<Question>(() => makeQuestion(ui.mode, Date.now() % 100000));
  ...
  const [battleEnemyHp, setBattleEnemyHp] = useState(ENEMIES[0].hp);
  ...
  useEffect(() => {
    window.localStorage.setItem(UI_KEY, JSON.stringify({ screen, mode }));
  }, [screen, mode]);

  const level = useMemo(() => levelFromXp(save.xp), [save.xp]);
  const progress = useMemo(() => xpProgress(save.xp), [save.xp]);
  const hasProfile = Boolean(save.player);
      ...
      <div className="shell">
        <header className="header">
          ...
          <div className="row" style={{ flex: "0 0 auto", gap: 10 }}>
            <button onClick={() => setScreen("home")} className="smallBtn">
              Volver al inicio
            </button>
            <button onClick={resetProgress} className="smallBtn">
              Reiniciar progreso
            </button>
          </div>
        </header>
            ...
            <section className="card">
              ...
              <div className="hero">
                <div className="heroTop">
                  ...
                  <div>
                    <div className="smallLabel">Bienvenido</div>
                    <h2 className="playerName">{save.player || "Jugador"}</h2>
                    <div className="muted">
                      Nivel {level} · {save.coins} monedas
                    </div>
                  </div>
                </div>
              ...
              <div style={{ marginTop: 16 }}>
                <div className="quickGrid" style={{ gridTemplateColumns: "repeat(4, minmax(0, 1fr))" }}>
                  <button onClick={() => setScreen("profile")} className="smallBtn">
                    👤 Perfil
                  </button>
                  <button onClick={() => setScreen("reward")} className="smallBtn">
                    🏆 Logros
                  </button>
                  <button onClick={() => startGame("training")} className="smallBtn">
                    🎒 Jugar
                  </button>
                  <button onClick={() => setMessage("Mapa aún en desarrollo.")} className="smallBtn">
                    🗺️ Mapa
                  </button>
                </div>
              </div>
              ...
              <section className="sideCard">
                ...
                <div style={{ marginTop: 12 }} className="row">
                  <input
                    ...
                    className="input"
                  />
                  <button onClick={saveName} className="mainBtn">
                    Guardar
                  </button>
                </div>
              </section>
              ...
              <section className="sideCard">
                ...
                <div className="modeRow">
                  {(["story", "training", "match", "boss"] as Mode[]).map((m) => (
                    <button key={m} onClick={() => setMode(m)} className={`pillBtn ${mode === m ? "active" : ""}`}>
                      {MODE_LABEL[m]}
                    </button>
                  ))}
                </div>
          ...
          <div className="grid">
            <section className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 12 }}>
                <div>
                  <div className="smallLabel">Combate</div>
                  <h2 className="playerName" style={{ fontSize: "2.2rem" }}>
              ...
              <div className="questionCard">
                ...
                <div className="actionRow">
                  ...
                  />
                  <button onClick={submitAnswer} className="mainBtn">
                    ATACAR
                  </button>
                </div>

                <div className="actionRow">
                  <button onClick={() => setShowHint((v) => !v)} className="smallBtn">
                    Pista
                  </button>
                  <button onClick={() => setAnswer(String(question.answer))} className="smallBtn">
                    Mostrar respuesta
                  </button>
                </div>
          ...
          <main className="rewardWrap">
            <div className="rewardCard">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                <h2 className="rewardTitle" style={{ margin: 0, fontSize: "2.2rem" }}>
                  Perfil
                </h2>
                <button onClick={() => setScreen("home")} className="smallBtn">
                  Cerrar
              ...
              <button onClick={resetProgress} className="smallBtn" style={{ marginTop: 18, width: "100%" }}>
                Reiniciar progreso
              </button>
            </div>
          </main>
    ...
    <div className="fighter">
      ...
      <div className="bar" style={{ marginTop: 10 }}>
        <div
          ...
          style={{
            width: `${pct}%`,
            background: good ? "linear-gradient(90deg,#36d399,#a7f3d0)" : "linear-gradient(90deg,#ff6b6b,#ff9c9c)",
          }}
        />
      </div>
    </div>
